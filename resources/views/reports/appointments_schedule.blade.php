...
@section('title', 'Reports Appointments schedule')
    ...
    <div class="panel panel-success">
        <div class="no-print panel-heading">
            <div class="row">
                ...
                <div class="col-md-4">
                    <strong>Reports Appointments schedule</strong>
                </div>
                <div class="col-md-4">
        ...
        <div class="panel-body">
            <form class="no-print" method="post" action="{{ route('reports.appointments.schedule') }}" id="appointment-form">
                @csrf
                <div class="form-row">
                    <div class="col-md-3">
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (data.length > 0) {
                        ...
                        if ($('#end_date').val() == "") {
                            $('#info').text('There ' + verb + ' ' + data.length + ' appointment(s) on ' + formatDate($('#appointment_date').val()) + ' for ' + $('#doctor_id option:selected').text());
                        } else {
                            $('#info').text('There ' + verb + ' ' + data.length + ' appointment(s) from ' + formatDate($('#appointment_date').val()) + ' to ' + formatDate($('#end_date').val()) + ' for ' + $('#doctor_id option:selected').text());
                        }
                        ...
                        data.forEach(function (value, index, array) {
                            $('#appointment').append(
                                ...
                                '<td class="r-table__cell" data-label="Patient"><a href="{{ url('patients') }}/' + value.user.id + '">' + value.user.firstname +' '+ value.user.lastname + '</a></td>\n' +
                                '<td class="r-table__cell break-word" data-label="Practitioner">' + value.doctor.firstname +' '+ value.doctor.lastname + '</td>' +
                                '<td class="r-table__cell break-word" data-label="Calendar"><a href="{{ url('calendar') }}" class="btn btn-info"><i class="fa fa-eye"></i></a></td>' +
                                '<td class="r-table__cell break-word" data-label="List"><a href="{{ url('appointments') }}" class="btn btn-info"><i class="fa fa-eye"></i></a></td>' +
                                '</tr>'
                            );
                        });
                        ...
                        if ($('#end_date').val() == "") {
                            $('#info1').text('There are no appointments on ' + formatDate($('#appointment_date').val()) + ' for ' + $('#doctor_id option:selected').text());
                        } else {
