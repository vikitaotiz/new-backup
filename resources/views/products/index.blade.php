...
@section('content')
    ...
    <div class="panel panel-success" id="products-wrapper" style="display: none">
        <div class="panel-heading">
          <div class="row">
            ...
            <div class="col-md-4">
                <a href="{{route('products.create')}}" class="btn btn-sm btn-primary btn-block"><i class="fa fa-plus"></i> Create New Product</a>
            </div>
        </div>
        </div>
        ...
        <div class="panel-body">
            <div class="row">
                <div class="col-md-1">
                    <select v-model="params.perPage" class="form-control" @change="getProducts()">
            ...
            <table class="table table-hover table-striped table-custom" v-if="products.length > 0">
                <thead>
                    <tr>
                        <th @click="filterParam('sort', 'code_serial_number')" :class="{ 'table-custom-header' : params.sort_by != 'code_serial_number', 'sorting_asc' : params.sort_by == 'code_serial_number' && params.order_by == 'asc', 'sorting_desc' : params.sort_by == 'code_serial_number' && params.order_by == 'desc' }">Code / Serial Number</th>
                        ...
                        <th @click="filterParam('sort', 'stock')" :class="{ 'table-custom-header' : params.sort_by != 'stock', 'sorting_asc' : params.sort_by == 'stock' && params.order_by == 'asc', 'sorting_desc' : params.sort_by == 'stock' && params.order_by == 'desc' }">Stock</th>
                        <th @click="filterParam('sort', 'buying_price')" :class="{ 'table-custom-header' : params.sort_by != 'buying_price', 'sorting_asc' : params.sort_by == 'buying_price' && params.order_by == 'asc', 'sorting_desc' : params.sort_by == 'buying_price' && params.order_by == 'desc' }">Buying Price</th>
                        <th @click="filterParam('sort', 'selling_price')" :class="{ 'table-custom-header' : params.sort_by != 'selling_price', 'sorting_asc' : params.sort_by == 'selling_price' && params.order_by == 'asc', 'sorting_desc' : params.sort_by == 'selling_price' && params.order_by == 'desc' }">Sellling Price</th>
                ...
                <tbody>
                    <tr v-for="product in products">
                        <td>@{{product.code_serial_number}}</td>
                        <td>@{{product.name}}</td>
                        <td>@{{product.stock}}</td>
                        <td>@{{product.buying_price}}</td>
                        <td>@{{product.selling_price}}</td>
                        <td>@{{product.created_at ? moment(product.created_at).format('ddd Do, MMM, YYYY') : 'N/A'}}</td>
                        <td>@{{product.doctor_name ? product.doctor_name : 'N/A'}}</td>
                        <td>
                            <div class="row">
                                <div class="col-md-4">
                                    <a :href="base_url + '/products/' + product.id" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> View</a>
                                </div>
                                <div class="col-md-8">
            ...
            <h4 v-else>There are no products</h4>
            <div class="row">
                <div class="col-md-6">
